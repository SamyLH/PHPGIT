...
$badges = $badgeSystem->getBadgesUtilisateur($user_id);
?>

<h2 class="text-2xl font-semibold mt-8 mb-4">🏅 Mes Badges</h2>
    ...
    <?php else: ?>
        <?php foreach ($badges as $b): ?>
            <div class="bg-white/10 p-3 rounded-xl text-center">
                <?php if ($b['icone']): ?>
                    <img src="<?= htmlspecialchars($b['icone']) ?>" alt="<?= htmlspecialchars($b['nom']) ?>" class="w-16 h-16 mx-auto mb-2">
                <?php endif; ?>
                <p class="font-semibold"><?= htmlspecialchars($b['nom']) ?></p>
                <p class="text-sm text-gray-300"><?= htmlspecialchars($b['description']) ?></p>
                <p class="text-xs mt-1">🎉 Obtenu le <?= date('d/m/Y', strtotime($b['date_obtenu'])) ?></p>
            </div>
        <?php endforeach; ?>
    <?php endif; ?>
</div>
